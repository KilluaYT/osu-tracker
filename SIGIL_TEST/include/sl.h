...
extern "C" {
	...
	SIGIL_API void slWindow(int width, int height, const char* title, int fullScreen, int refreshRate, bool vsync, bool resizable);
	SIGIL_API void slShowCursor(int showCursor);
	SIGIL_API void slClose();
	SIGIL_API int slShouldClose();
	 void slSetRefreshRate(int HZ, bool vsync);
